...
class Graph:
    ...
    def add_a_node(self, node_name):
        ...
        self.adjency_list[str(node_name)] = list()
    ...
    def add_an_edge(self, from_node, to_node, value):
        ...
        if not(to_node in self.nodes):
            ...
            return False
        if to_node in self.adjency_list[str(from_node)]:
            return False
        self.edges.append([from_node, to_node, value])
        self.edges.append([to_node, from_node, value])
        self.adjency_list[str(from_node)].append(to_node)
        self.adjency_list[str(to_node)].append(from_node)
